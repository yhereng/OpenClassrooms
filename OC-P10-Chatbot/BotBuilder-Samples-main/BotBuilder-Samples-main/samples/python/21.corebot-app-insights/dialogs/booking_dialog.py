    ...
    async def start_date_step(
        ...
        if booking_details.str_date is None:          
            return await step_context.prompt(
                TextPrompt.__name__,
                PromptOptions(
                    prompt=MessageFactory.text("When will you start travelling?")
                ),
    ...
    async def end_date_step(
        ...
        if booking_details.end_date is None:          
            return await step_context.prompt(
                TextPrompt.__name__,
                PromptOptions(
                    prompt=MessageFactory.text("When will you end travelling?")
                ),
            )  # pylint: disable=line-too-long
    ...
    async def budget_step(
        ...
        if booking_details.budget is None:          
            return await step_context.prompt(
                TextPrompt.__name__,
                PromptOptions(
                    prompt=MessageFactory.text("What would be your budget for travelling?")
                ),
            )  # pylint: disable=line-too-long
